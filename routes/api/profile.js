...
router.post('/',[auth,[check('status','Status is required').not().isEmpty(),check('skills','Skills is required').not().isEmpty()]],async(req,res)=>{
    ...
    const {company,bio,status,githubusername,website,location,skills,youtube,facebook,twitter,instagram,linkedin}=req.body;
...
async(req,res)=>{
  ...
  const {title,from,to,current,company,location,description}=req.body;
...
async(req,res)=>{
  ...
  const {school,from,to,current,degree,location,description,fieldofstudy}=req.body;
  const newEdu={school,fieldofstudy,from,to,current,location,degree,description};
  try{
    const profile=await Profile.findOne({user:req.user.id});
